...
Start nested loops for variables a, b, and c  
Check whether a, b, and c satisfy the given rules 
If they do, calculate the product and store it in prod 
End all loops
Display prod
...
Declare Integer i
assign 0 to i
iterate starting from i to 1000 with an increment 1
	assign i to the a
	Declare Integer j
	assign 0 to j
	iterate starting from j to 1000 with an increment 1
		assign j to the b
		Declare Integer k
		assign 0 to k
		iterate starting from k to 1000 with an increment 1
			assign k to the c
			If (a less than b) And(c greater than b) And (c greater than a) And (sum of abc is equal to 1000) And (a to the power of 2 plus b to the power of 2 equals c to the power of 2) Then
